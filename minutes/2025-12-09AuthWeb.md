...
[dokie.li](http://dokie.li) doesn’t intend to say what’s true or false, just identify things to look into. Credibility assessment is performed by individuals or communities using dokieli's indicators, rather than dokieli itself. This design encourages digital literacy and supports independent critical thinking.


Sarven: There’s some peer reviewed background work on dokieli, also the [Linked Data Notifications](https://www.w3.org/TR/ldn/) is a W3C Recommendation.
...
Sarven: Background on TimBL’s “[Oh Yeah](https://www.w3.org/DesignIssues/UI#OhYeah)” button as discussed in his Design issues. If you’re curious as a reader about a statement on a webpage and you want provenance our a source check, who signed the statement. Tim mentioned this in his design issue in 1997. Dokieli is a realisation of that. The flow is simple, the user highlights a sentence, then there’s a Oh Yeah? button, which opens a panel that shows content analysis: what kind of knowledge is it, topics, entities, sentiments and content warnings that should be shown. All this is in the context of the user’s online profile. There’s some form of validation and a way of checking if content is missing citations or links. The fact checking can be done through authoritative sources, which can mean different things to different people. The commons on the web might be wikipedia / wikidata, open street maps, nano publications, whois lookups, or claim checks of statements for authenticity or validity by asking your social network or domain experts.

This is implemented. The source code is on the web. [Shows screencast]

The demo shows an article, with some annotation already. The user is signed-in. The user wants to understand the annotation, they click and it to view. The statement was "The evidence shows that the Earth is flat". And the annotation says "On the contrary, the Earth is not flat." The annotation is stored wherever the annotator decided to store it, e.g., their own storage or an annotation service, but it’s not tied to the article.

The “Oh Yeah?” Button opens a tab, to request an assessment from a social network or shared inbox or get a hold of domain experts on topic. Assessment can include tags. The user profile indicates topics they’re interested in, and where to share notifications to, and where to get annotations from. In this case they happen to be interested in climate change, so going to sent to an inbox for climate change topic, for example. The annotations there are from different locations. Notice that the highlighter shows different marks, one is asking for specificity, help, or questioning that statement.

In another example, the user selects text, clicks Oh Yeah? And the panel shows claim checks. Some processing happened on the statement. The point at the moment is not so much that the datasets are trained on this particular content, but we picked something to get something out of the box that could be customised to the kind of content. A physics, biologist, teacher may be working with different datasets. This all depends on where the user profile might be or where their profile is pointing to, and which claim check / endpoint they want to use and trust. There’s a confidence level that it’s a factual statement, with an explanation. It will say different things for different kinds of sentences.

We look up wikidata for the entities identified. We use SPARQL as it seemed much more efficient and geared to filtering to the kinds of things we want to filter on, as opposed to the API which is also available. Useful way to find content.

Nanopublications for particular statement has assertions, provenance, and publication info. Small facts of information that can be looked up.

Working on verifiable credentials, to check statements that may be publicised or accessible to the reading. We do a whois lookup on who’s behind a domain. CredWeb CG report includes many things demonstrated by this authoring tool - dokieli.

There’s a graph view, which shows all the information dokieli was able to gather from claim check, source of the claim, wikidata, nanopub and so on. There may be other facts that's relevant or connectable. This data itself could be published on its own, and its queryable like everything else.

Dokieli uses many web standards: from protocols to notifications, web annotation, ActivityPub, activitystreams, webid, type indexes, web access control, ODRL, Memento (for time traveling resources), Robust Links (where content may have changed), use SPARQL queries, typed citations, e.g., linking to something for evidence or to refute an argument somewhere else on the web. This is all specific to units of information - sentence level as opposed to document. We use the PROV ontology.

New standards, potentially, a credibility assessment model, argumentation data model. We’re interested in getting more granular web annotation motivations, beyond questioning. Web Extensions. Dokieli could even be part of the browser itself, since the web platform didn't come with authenticity features, so we want to see how it works as an application or as part of the browser.

On security, we have a threat model. Who’s monitoring the inboxes, or writing what annotations. How to identify biases, address harassment. Moderation is always a concern.
